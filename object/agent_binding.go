...
// @hanzo/bot runtime for a given agent. `buildBotUserData`'s launch path is the
// single writer; reconcile is the single reader. The value is the agent name so
// one machine hosts exactly one agent's bot (the machine IS the bot's identity).
const BotRuntimeTag = "hanzo-bot"

// AgentBinding records that a machine runs the @hanzo/bot runtime for a specific
// cloud Agent. It is the visor half of the Bot lifecycle: a Bot = Agent
...
// machine's launch cloud-init (`buildBotUserData`), which the operator drives
// via LaunchMachine with a `hanzo-bot:<agent>` tag before calling this. BindAgent
// records the agent↔machine relation and reports the observed convergence state.
